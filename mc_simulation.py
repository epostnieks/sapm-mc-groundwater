#!/usr/bin/env python3
"""
SAPM Monte Carlo Simulation — Groundwater (Ogallala) / Recharge Floor
=======================================

Reproduces all quantitative results in:

  Postnieks, E. (2026). SAPM Working Paper: Groundwater (Ogallala) (Recharge Floor).
  Available: SSRN.
...
CONFIG = {
    "paper_id": "groundwater",
    "domain_name": "Groundwater (Ogallala)",
    "theorem": "Recharge Floor",
    "classification": "Class 1 \u2014 Impossibility",
    "seed": 42,
    "n_draws": 100000,
    "correlation_rho": 0.3,
    "private_payoff_B": 9.5,
    "channels": {
        "C1_irrigation_premium": {
            ...
            "high": 10.0,
            "weight": 0.18,
            "description": "Yield differential loss ($2.1B), land value depreciation ($1.5B), transition costs ($1.4B), pumping escalation ($0.8B). Sources: World Bank, Deines et al. (2020), Sampson et al. (2019), USDA ERS."
        },
        "C2_rural_economic_cascade": {
            ...
            "high": 6.0,
            "weight": 0.1,
            "description": "Multiplier losses from feedlot/meatpacking relocation ($1.8B), municipal infrastructure stranding ($0.8B), population displacement ($0.6B). Sources: Hornbeck-Keskin (2012), IMPLAN, USDA ERS."
        },
        "C3_ecological_decoupling": {
            ...
            "high": 5.0,
            "weight": 0.08,
            "description": "530 miles stream habitat lost ($1.2B), riparian destruction ($0.5B), saline intrusion ($0.4B), land subsidence ($0.4B). Sources: Perkin et al. (2017), NOAA benefit transfer."
        },
        "C4_climate_acceleration": {
            ...
            "high": 8.0,
            "weight": 0.13,
            "description": "Climate-driven demand increase ($1.5B), thermal stress yield loss ($1.2B), accelerated depletion ($0.9B), lost adaptation value ($0.5B). Sources: Crosbie et al. (2013), CMIP6."
        },
        "C5_intergenerational_asset_destruction": {
            ...
            "high": 22.0,
            "weight": 0.4,
            "description": "Asset destruction ($4.2B from Hornbeck-Keskin $16B capitalized loss), Hotelling stock cost ($5.3B), drought insurance risk ($2.0B), food security externality ($1.3B). Dominant channel (40%)."
        },
        "C6_governance_failure": {
            ...
            "high": 7.0,
            "weight": 0.12,
            "description": "Jevons Paradox from EQIP ($1.2B), crop insurance moral hazard ($0.9B from Sloggy et al. 2025), regulatory capture ($0.8B), Rule of Capture externality ($0.9B)."
        }
    },
    "impossibility_floor": 1.5
}
